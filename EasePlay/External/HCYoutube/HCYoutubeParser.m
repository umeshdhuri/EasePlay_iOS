        ...
        if (!error) {
            ...
            if (parts) {
                ...
                /*
                 ...
                 NSArray *fmtStreamMapArray1 = [[NSArray alloc] init];
                 fmtStreamMapArray1 = [videoEncodedString componentsSeparatedByString:@"="];
                 
                 if([fmtStreamMapArray1 count] >= 2) {
                 NSString *typeVal = [[fmtStreamMapArray1 objectAtIndex:1] stringByDecodingURLFormat];
                 [videoDictionary setObject:typeVal forKey:[fmtStreamMapArray1 objectAtIndex:0]];
                 
                 }
                 
                 }
                 
                 for (NSString *key in [videoDictionary allKeys])
                 {
                 if([key isEqualToString:@"url"]) {
                 [videoDictionaryVal setObject:[videoDictionary valueForKey:key] forKey:@"playURL"] ;
                 }
                 //do stuff with feed.
                 }
                 
                 return videoDictionaryVal ;
